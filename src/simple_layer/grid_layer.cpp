    ...
    if (new_map_){
      ...
      for (grid_map::GridMapIterator iterator(map_); !iterator.isPastEnd(); ++iterator) {
        ...
        if (!std::isnan(trav_value)) {
          ...
          if (trav_value >= -2.9 /*&& uncert_data(i) >= 1*/) {  // traversability: (-3.5, -2.1]  (and uncertainty range: [1.0, +inf)  ) --> LETHAL_OBSTACLE
            cost = FREE_SPACE;
          }
          else if (trav_value >= -4 /*&& uncert_data(i) < 1.0*/) {  // traversability: (-4, -3.5] (and uncertainty range: (0.4, 1.0) ) --> INSCRIBED_INFLATED_OBSTACLE
          ...
          cost = std::min(double(LETHAL_OBSTACLE), 2.0 * LETHAL_OBSTACLE * (trav_value - 0.1));
